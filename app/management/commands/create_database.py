...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        # que_create = [Question(title=f"title #{i-1}", text_que=f"text for que{i-1}", person_que=Profile.objects.get(id=choice(profiles))) for i in range(120000)]
        ...
        ans_create = [Answer(text_ans=f"text for ans{i}", person_ans=Profile.objects.get(id=choice(profiles)), que=Question.objects.get(id=choice(questions))) for i in range(12000)]
        Answer.objects.bulk_create(ans_create)
        answers = list(Answer.objects.values_list("id", flat=True))

        like_to_ans = [LikeToAns(like_ans=Answer.objects.get(id=choice(answers)), person_like_ans=Profile.objects.get(id=choice(profiles))) for i in range(12000)]
        LikeToAns.objects.bulk_create(like_to_ans)

        like_to_que = [LikeToQue(like_que=Question.objects.get(id=choice(questions)), person_like_que=Profile.objects.get(id=choice(profiles))) for i in range(12000)]
